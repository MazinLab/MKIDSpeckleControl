...
class Calibrator(object):
    ...
    def __init__(self, dmChanName, sharedImageName, useWvl=False, wvlStart=700, wvlStop=1400):
        ...
        self.params = propertytree.PTree(cfgFile)
        self.shmImage.useWvl = useWvl
        if useWvl:
            self.shmImage.wvlStart = wvlStart
            self.shmImage.wvlStop = wvlStop

    def run(start, end, amplitude, nPoints=10, integrationTime=5):
        ...
        for i in range(nPoints):
            ...
            self.dm.updateDM()
            self.shmImage.startIntegrating(0, integrationTime)
            image = self.shmImage.receiveImage()
            calgui = CalspotGUI(image)
            self.speckLocs[i*2:i*2+1, :, :] = calgui.speckLocs
...
class CalspotGUI(object):
    ...
    def onClick(self, event):
        ...
        if speckNum == 2:
            ...
            print 'Done. If you want to start over, click on the first speckle pair, else close
                    this gui'
